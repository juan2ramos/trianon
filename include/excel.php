<?php
include('env.php');
include('Connection.php');

$connection = Connection::getInstance($dataBase);
$sqlTxt = 'select * from `User-List` ' ;
$users = $connection->query($connection->run($sqlTxt));



error_reporting(E_ALL);
ini_set('display_errors', TRUE);
ini_set('display_startup_errors', TRUE);
date_default_timezone_set("America/Bogota");
if (PHP_SAPI == 'cli')
    die('This example should only be run from a Web Browser');
/** Include PHPExcel */
require_once dirname(__FILE__) . '/PHPExcel/Classes/PHPExcel.php';
// Create new PHPExcel object
$objPHPExcel = new PHPExcel();
// Set document properties
$objPHPExcel->getProperties()->setCreator("Juan2Ramos")
    ->setLastModifiedBy("")
    ->setTitle("Office 2007 XLSX Test Document")
    ->setSubject("Office 2007 XLSX Test Document")
    ->setDescription("")
    ->setKeywords("office 2007 openxml php")
    ->setCategory("");

$i = 2;
foreach($users as $user){

    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('A1', 'id')
        ->setCellValue('B1', 'name')
        ->setCellValue('C1', 'email')
        ->setCellValue('D1', 'phone')
        ->setCellValue('E1', 'number')
        ->setCellValue('F1', 'date');
    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('A'.$i, $user['id'])
        ->setCellValue('B'.$i, $user['name'])
        ->setCellValue('C'.$i, $user['email'])
        ->setCellValue('D'.$i, $user['phone'])
        ->setCellValue('E'.$i, $user['number-identification'])
        ->setCellValue('D'.$i, $user['date']);
$i++;
}


$objPHPExcel->getActiveSheet()->setTitle('lista de inscritos ');
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel5)
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="lista_inscritos.xls"');
header('Cache-Control: max-age=0');
// If you're serving to IE 9, then the following may be needed
header('Cache-Control: max-age=1');
// If you're serving to IE over SSL, then the following may be needed
header ('Expires: Mon, 26 Jul 1997 05:00:00 GMT'); // Date in the past
header ('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT'); // always modified
header ('Cache-Control: cache, must-revalidate'); // HTTP/1.1
header ('Pragma: public'); // HTTP/1.0
$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
$objWriter->save('php://output');
exit;